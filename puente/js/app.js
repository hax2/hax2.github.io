...
function speak(text, rateOverride, onEnd) {
  cancelSpeech();
  if (!("speechSynthesis" in window)) {
    if (onEnd) onEnd();
    return;
  ...
  try {
    ...
    if (onEnd) {
      ...
      fallbackTimer = setTimeout(done, Math.max(2000, words * 800 + 1000));
    }
...
function fusionComplete(wordToPlayFirst) {
  if (graded) return;
  graded = true;
...
$("btnHelp").onclick = () => { cancelSpeech(); openModal("helpOverlay"); };
$("btnStart").onclick = () => {
